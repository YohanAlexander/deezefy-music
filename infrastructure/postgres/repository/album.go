...
func getAlbum(id int, db *sql.DB) (*entity.Album, error) {
	stmt, err := db.Prepare(`select email, senha, data_nascimento, nome_artistico, biografia, ano_formacao, id, titulo, ano_lancamento from deezefy.Album
	...
	stmt, err = db.Prepare(`select email, senha, data_nascimento, primeiro_nome, sobrenome from deezefy.Album
...
func (r *AlbumPSQL) Update(e *entity.Album) error {
	_, err := r.db.Exec(`update deezefy.Album set id = ?, titulo = ?, ano_lancamento = ?
	where id = ?`, e.ID, e.Titulo, e.AnoLancamento)
	if err != nil {
		return err
	}
	// update related ouvinte
	_, err = r.db.Exec(`delete from deezefy.Ouvinte_Salva_Album where fk_album = ?`, e.ID)
	if err != nil {
		return err
	}
	for _, b := range e.Salvou {
		_, err := r.db.Exec(`insert into deezefy.Ouvinte_Salva_Album
		(fk_ouvinte, fk_artista, fk_album) values(?,?,?)`, b.Usuario.Email, e.Artista.Usuario.Email, e.ID)
		if err != nil {
			return err
		}
	}
	// update related musica
	_, err = r.db.Exec(`delete from deezefy.Album_Contem_Musica where fk_album = ?`, e.ID)
	if err != nil {
		return err
	}
	for _, b := range e.Musicas {
		_, err := r.db.Exec(`insert into deezefy.Album_Contem_Musica
		(fk_musica, fk_artista, fk_album) values(?,?,?)`, b.ID, e.Artista.Usuario.Email, e.ID)
		if err != nil {
			return err
		}
	}
...
func (r *AlbumPSQL) Search(query string) ([]*entity.Album, error) {
	stmt, err := r.db.Prepare(`select titulo from deezefy.Album where titulo like ?`)
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
...
func (r *AlbumPSQL) List() ([]*entity.Album, error) {
	stmt, err := r.db.Prepare(`select id from deezefy.Album`)
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
...
func (r *AlbumPSQL) Delete(id int) error {
	_, err := r.db.Exec(`delete from deezefy.Album where id = ?`, id)
	if err != nil {
		return err
	}
	return nil
